...
// GET /api/users/:id/chat-settings — owner or admin
router.get('/users/:id/chat-settings', auth, ownerOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
...
// PUT /api/users/:id/chat-settings — owner or admin
router.put('/users/:id/chat-settings', auth, ownerOrAdmin, async (req, res) => {
    const { id } = req.params;
    const {
        bubbleTheme,
        myBubbleColor,
...
// GET /api/users/:id/moderation — owner or admin
router.get('/users/:id/moderation', auth, ownerOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.execute({
...
// PUT /api/users/:id/moderation/:targetId — owner or admin
router.put('/users/:id/moderation/:targetId', auth, ownerOrAdmin, async (req, res) => {
...
// GET /api/users/:id/room-role/:roomId — authenticated
router.get('/users/:id/room-role/:roomId', auth, async (req, res) => {
...
// PUT /api/users/:id/room-role/:roomId — authenticated, uses req.user for permission check
router.put('/users/:id/room-role/:roomId', auth, async (req, res) => {
    const { id, roomId } = req.params;
    const { role } = req.body || {};
    ...
    try {
        // Use authenticated user from token, not from request body
        const requestingUserId = req.user.id;
        const requesterRole = await db.execute({
            sql: 'SELECT role FROM user_room_roles WHERE user_id = ? AND room_id = ?',
            args: [requestingUserId, roomId],
        });
        const requesterIsAdmin = req.user.isAdmin || (requesterRole.rows.length && (requesterRole.rows[0].role === 'owner' || requesterRole.rows[0].role === 'mod'));
        if (!requesterIsAdmin) {
...
// GET /api/users/:id/mentions — owner or admin
router.get('/users/:id/mentions', auth, ownerOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.execute({
...
// POST /api/users/:id/mentions/mark-read — owner or admin
router.post('/users/:id/mentions/mark-read', auth, ownerOrAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    if (safeIds.length === 0) {
        return res.status(400).json({ error: 'No valid mention IDs' });
    }
    try {
...
// POST /api/users/:id/ban — ban from room, authenticated mod/owner check uses token
router.post('/users/:id/ban', auth, async (req, res) => {
    const { id } = req.params;
    const { roomId, reason, durationMinutes } = req.body || {};
    if (!roomId) return res.status(400).json({ error: 'roomId required' });

    const banningUserId = req.user.id;
    ...
    try {
        ...
        const isMod = req.user.isAdmin || (bannerRole.rows.length && (bannerRole.rows[0].role === 'owner' || bannerRole.rows[0].role === 'mod'));
        if (!isMod) {
...
// DELETE /api/users/:id/ban/:roomId — unban, authenticated mod/owner uses token
router.delete('/users/:id/ban/:roomId', auth, async (req, res) => {
    const { id, roomId } = req.params;
    const requestingUserId = req.user.id;
    try {
        const requesterRole = await db.execute({
        ...
        const isMod = req.user.isAdmin || (requesterRole.rows.length && (requesterRole.rows[0].role === 'owner' || requesterRole.rows[0].role === 'mod'));
        if (!isMod) {
...
// GET /api/users/:id/emoji-favorites — authenticated
router.get('/users/:id/emoji-favorites', auth, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.execute({
...
// POST /api/users/:id/emoji-favorites/:emoji — owner or admin
router.post('/users/:id/emoji-favorites/:emoji', auth, ownerOrAdmin, async (req, res) => {
...
// GET /api/users/:id/audit-log/:roomId — authenticated, mod/owner of room
router.get('/users/:id/audit-log/:roomId', auth, async (req, res) => {
